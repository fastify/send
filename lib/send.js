...
function normalizeOptions (options) {
  ...
  // hooks
  const onSendDirectory = typeof options.onSendDirectory === 'function'
    ? options.onSendDirectory
    : null
  const onSendFile = typeof options.onSendFile === 'function'
    ? options.onSendFile
    : null
  const onSendError = typeof options.onSendError === 'function'
    ? options.onSendError
    : null
...
function sendError (statusCode, err, options) {
  if (options.onSendError) {
    return options.onSendError(statusCode, err)
  }
...
function sendStatError (err, options) {
  ...
  switch (err.code) {
    ...
    case 'ENOTDIR':
    case 'ENOENT':
      return sendError(404, err, options)
    default:
      return sendError(500, err, options)
  }
  /* c8 ignore stop */
}
...
function sendFileDirectly (request, path, stat, options) {
  if (options.onSendFile) {
    return options.onSendFile(path, stat)
  }
  ...
  if (isConditionalGET(request)) {
    if (isPreconditionFailure(request, headers)) {
      return sendError(412, null, options)
    }
...
function sendRedirect (path, options) {
  if (options.onSendDirectory) {
    return options.onSendDirectory(path)
  }

  if (hasTrailingSlash(options.path)) {
    return sendError(403, null, options)
  }
...
async function sendIndex (request, path, options) {
  ...
  if (err) {
    return sendStatError(err, options)
  }

  return sendError(404, null, options)
}
...
async function sendFile (request, path, options) {
  ...
  if (error) return sendStatError(error, options)
...
async function send (request, _path, options) {
  ...
  if (parsed.statusCode !== undefined) {
    return sendError(parsed.statusCode, null, opts)
  }
  ...
  if (
    ...
    switch (opts.dotfiles) {
      ...
      case 2: // 'deny'
        ...
        return sendError(403, null, opts)
      case 1: // 'ignore'
      default:
        debug('ignore dotfile "%s"', path)
        return sendError(404, null, opts)
    }
  }
