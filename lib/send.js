...
function normalizeOptions (options) {
  ...
  const maxReturnSize = options.maxReturnSize !== undefined
    ? Number(options.maxReturnSize)
    : null
    ...
    if (
      ...
      if (isRangeFresh(request, headers)) {
        ...
        if (ranges.length === 0) {
          ...
          // Content-Range
          statusCode = 206
          if (options.maxReturnSize) {
            ranges[0].end = Math.min(ranges[0].end, ranges[0].start + options.maxReturnSize - 1)
          }
