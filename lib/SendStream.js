...
function SendStream (req, path, options) {
  ...
  this._maxage = typeof this._maxage === 'string'
    ...
    : Number(this._maxage)
  this._maxage = this._maxage === this._maxage // eslint-disable-line no-self-compare
...
SendStream.prototype.isPreconditionFailure = function isPreconditionFailure () {
  ...
  if ('if-unmodified-since' in req.headers) {
    ...
    if (unmodifiedSince === unmodifiedSince) { // eslint-disable-line no-self-compare
      const lastModified = Date.parse(res.getHeader('Last-Modified'))
      if (
        lastModified !== lastModified || // eslint-disable-line no-self-compare
...
SendStream.prototype.isRangeFresh = function isRangeFresh () {
  ...
  if (ifRangeTimestamp !== ifRangeTimestamp) { // eslint-disable-line no-self-compare
    return false
  }
  ...
  if (this._acceptRanges) {
    ...
    if (
      ...
      if (this.isRangeFresh()) {
        // parse
        const ranges = parseRange(len, rangeHeader)
