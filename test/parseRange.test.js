...
test('parseRange', function (t) {
  ...
  t.test('should return empty array if all specified ranges are invalid', function (t) {
    t.plan(3)
    t.strictSame(parseRange(200, 'bytes=500-20'), [])
    t.strictSame(parseRange(200, 'bytes=500-999'), [])
    t.strictSame(parseRange(200, 'bytes=500-999,1000-1499'), [])
  })
  ...
  t.test('should retain original order /1', function (t) {
    t.plan(3)
    const range = parseRange(150, 'bytes=90-99,5-75,100-199,101-102,0-4')
    t.equal(range.length, 2)
